...
function upstart(conf,outdir){
    ...
    var out  = "";
    render((conf.template || 'upstart') + '/foreman.conf', conf, writeout(path) )
}

function upstart_app(conf,outdir){
    var out = "";
    var path = outdir + "/" + conf.application + "-" + conf.process + ".conf";
    render((conf.template || 'upstart') + '/foreman-APP.conf', conf, writeout(path) )
}

function upstart_app_n(conf,outdir){
    var out = "";
    var path = outdir + "/" + conf.application + "-" + conf.process + "-" + conf.number + ".conf";
    render((conf.template || 'upstart') + '/foreman-APP-N.conf', conf, writeout(path) )
}
...
function systemd(conf,outdir){
    ...
    var out  = "";
    render((conf.template || 'systemd') + '/foreman.target', conf, writeout(path) )
}

function systemd_app(conf,outdir){
    var out = "";
    var path = outdir + "/" + conf.application + "-" + conf.process + ".target";
    render((conf.template || 'systemd') + '/foreman-APP.target', conf, writeout(path) )
}

function systemd_app_n(conf,outdir){
    var out = "";
    var path = outdir + "/" + conf.application + "-" + conf.process + "-" + conf.number + ".service";
    render((conf.template || 'systemd') + '/foreman-APP-N.service', conf, writeout(path) )
}
